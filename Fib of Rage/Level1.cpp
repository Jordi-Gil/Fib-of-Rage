...
Level1::Level1(int left, int right, int bottom, int top) : Level(left, right, bottom, top)
{
	max_x = 1160;
	min_x = left;
	collisions = NULL;
...
void Level1::render()
{
	...
	for each (Player *player in characters)
	{
		...
		if(player->getState() != 5) player->render();
	}
...
bool Level1::setBackground(const string &filename)
{
	glm::vec2 geom1[2] = { glm::vec2(0.f, 0.f), glm::vec2(2080.f, 547.f) };
...
void Level1::setMainPlayer() {
	...
	if (userPlayer == RYU) {
		...
		mainPlayer->init(glm::ivec2(SCREEN_X, SCREEN_Y), texProgram, "Resources/Sprites/Ryu/ryu.png", animations, glm::ivec2(P_WIDTH, P_HEIGHT), glm::vec2(1 / 27.f, 0.5f), USER_PLAYER, NULL, RYUT, 10000);
...
void Level1::setBossAnimations()
{
	...
	animations.clear();
	animations.resize(10);
	animations[0]  = make_pair(ENE_SR,  vector < glm::vec2 > {	glm::vec2(0.0f,      0.0f), glm::vec2(1  / 16.f, 0.0f), glm::vec2(2  / 16.f, 0.0f)});
	animations[1]  = make_pair(ENE_SL,  vector < glm::vec2 > {	glm::vec2(15 / 16.f, 0.5f), glm::vec2(14 / 16.f, 0.5f), glm::vec2(13 / 16.f, 0.5f)});
	animations[2]  = make_pair(ENE_MR,  vector < glm::vec2 > {	glm::vec2(3  / 16.f, 0.0f), glm::vec2(4  / 16.f, 0.0f), glm::vec2(5  / 16.f, 0.0f), glm::vec2(6  / 16.f, 0.0f)});
	animations[3]  = make_pair(ENE_ML,  vector < glm::vec2 > { glm::vec2(12 / 16.f, 0.5f), glm::vec2(11 / 16.f, 0.5f), glm::vec2(10 / 16.f, 0.5f), glm::vec2(9  / 16.f, 0.5f)});
	animations[4]  = make_pair(ENE_PR, vector < glm::vec2 > {	glm::vec2(9  / 16.f, 0.0f), glm::vec2(10 / 16.f, 0.0f), glm::vec2(11 / 16.f, 0.0f)});
	animations[5]  = make_pair(ENE_PL, vector < glm::vec2 > {	glm::vec2(6  / 16.f, 0.5f), glm::vec2(5  / 16.f, 0.5f), glm::vec2(4  / 16.f, 0.5f)});
	animations[6]  = make_pair(ENE_HR,  vector < glm::vec2 > {	glm::vec2(12 / 16.f, 0.0f)});
	animations[7]  = make_pair(ENE_HL,  vector < glm::vec2 > {	glm::vec2(3  / 16.f, 0.5f)});
	animations[8] = make_pair(ENE_DR,  vector < glm::vec2 > {	glm::vec2(13 / 16.f, 0.0f), glm::vec2(14 / 16.f, 0.0f), glm::vec2(15 / 16.f, 0.0f)});
	animations[9] = make_pair(ENE_DL,  vector < glm::vec2 > {	glm::vec2(2  / 16.f, 0.5f), glm::vec2(1  / 16.f, 0.5f), glm::vec2(0.0f,      0.5f)});
	
	bossPlayer = new Player();
	bossPlayer->init(glm::ivec2(SCREEN_X, SCREEN_Y), texProgram, "Resources/Sprites/Bear/bear_boss_1.png", animations, glm::ivec2(P_WIDTH, P_HEIGHT), glm::vec2(1 / 16.f, 0.5f), IA_PLAYER, mainPlayer, B1, 300);
...
void Level1::restartLevel(int player)
{
...
void Level1::moveCamera(int left, int right, int bottom, int top) {
...
void Level1::avanzaLevel()
{
	...
	for each (Player *p in characters)
	{
		...
		if (p->charType == A) p->setHP(100);
		else if (p->charType == J) p->setHP(70);
		else if (p->charType == Z) p->setHP(50);
		offset_y += 10;
	}
